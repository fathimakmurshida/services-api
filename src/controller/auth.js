  ...
  try {
    if (req.body.phonenumber) {
      ...
      Logindata.find({ phonenumber: phone }).then((response) => {
        if (response.length==0){ msg = "register";
      }else {msg = "login";
       } });
    }, 1000);
      setTimeout(() => {
        if (msg == message) {
          var options = {
            ...
            message: `Security code For ARCLIF is ${otp}`,
            numbers: [phone],
          };
          fast2sms
            .sendMessage(options)
            ...
            .catch((err) => {
              console.error(err);
              res.status(404).send({ msg: "fast2sms error", status: "404" });
            });
        } else {
        ...
        Logindata.find({ phonenumber: phone })
          .then((response) => {
            if (response.length == 0) {
              ...
              loginData.save().then((responsedata) => {
                msg = "register";
                details = [
                  {
                    data: responsedata,
